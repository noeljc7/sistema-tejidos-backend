          ...
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-end">
              ...
              <span className="text-[9px] text-slate-400 mt-1 font-mono uppercase">
                ID: {process.env.NEXT_PUBLIC_API_URL?.split('.')[0].replace('https://', '') || 'LOCAL'}
              </span>
            </div>
            <button 
        ...
        <section className="bg-white p-8 rounded-xl shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleRegisterTejedora} className="space-y-6">
            <input 
              ...
              className="w-full p-4 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:border-indigo-500 outline-none transition-all text-xs font-bold"
              value={nombre}
            ...
            <button 
              disabled={cargando}
              className={`w-full py-4 rounded-lg font-bold text-[10px] uppercase tracking-[0.3em] shadow-md transition-all ${cargando ? 'bg-slate-300 cursor-not-allowed text-slate-500' : 'bg-slate-900 hover:bg-black text-white'}`}
            >
          ...
          <h3 className="text-[10px] font-bold mt-14 mb-6 uppercase tracking-[0.3em] text-slate-400 border-b pb-4 flex justify-between items-center">
            PERSONAL REGISTRADO
            {selectedTejedoraId && (
              <button onClick={() => setSelectedTejedoraId(null)} className="text-indigo-600 font-black">VER TODOS</button>
            )}
          </h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
            {tejedoras.map((t) => (
              <div 
                ...
                className={`group relative flex flex-col items-center p-3 rounded-xl border transition-all cursor-pointer ${selectedTejedoraId === t.id.toString() ? 'border-indigo-600 bg-indigo-50' : 'border-slate-100 bg-white'}`}
              >
                <button 
                  onClick={(e) => { e.stopPropagation(); handleDeleteTejedora(t.id); }}
                  className="absolute -top-2 -right-2 bg-white text-slate-400 w-6 h-6 rounded-full border border-slate-200 opacity-0 group-hover:opacity-100 z-30 flex items-center justify-center text-[10px]"
                >✕</button>
                ...
                <span className="text-[9px] font-bold text-slate-600 truncate w-full text-center uppercase">{t.nombre}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-white p-8 rounded-xl shadow-sm border border-slate-200">
          ...
          <form onSubmit={handleRegisterProducto} className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <input type="number" placeholder="PESO" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold uppercase" value={peso} onChange={(e) => setPeso(e.target.value)} />
              <input type="number" placeholder="VALOR" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold uppercase" value={precio} onChange={(e) => setPrecio(e.target.value)} />
            ...
            <button 
              disabled={cargando}
              className={`w-full py-4 rounded-lg font-bold text-[10px] uppercase tracking-[0.3em] shadow-md transition-all ${cargando ? 'bg-slate-300' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}`}
            >
              INGRESAR AL CATÁLOGO
            </button>
          </form>

          <h3 className="text-[10px] font-bold mt-14 mb-6 uppercase tracking-[0.3em] text-slate-400 border-b pb-4 text-emerald-600">INVENTARIO</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {productos
              ...
              const tejedora = tejedoras.find(t => t.id === p.tejedora_id);
              return (
                <div key={p.id} className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm group">
                  <div className="relative h-44 w-full bg-slate-100">
                    <img src={p.foto_producto} alt={p.tipo} className="w-full h-full object-cover" />
                    <div className="absolute top-3 right-3 bg-white/95 px-3 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest text-slate-700">{p.color}</div>
                  </div>
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-4">
                      <p className="font-bold text-xs text-slate-800 uppercase">{p.tipo}</p>
                      <span className="text-[8px] px-2 py-1 rounded font-black tracking-widest bg-emerald-100 text-emerald-700">{p.estado?.toUpperCase()}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      <div className="bg-slate-50 p-2 rounded flex flex-col border border-slate-100">
                        <span className="text-[7px] text-slate-400 font-black uppercase">Gramos</span>
                        <span className="text-xs font-bold text-slate-700">{p.peso_gramos}g</span>
                      </div>
                      <div className="bg-slate-50 p-2 rounded flex flex-col border border-slate-100">
